...
export class EditProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.translate
      .get('PROFILE.EDIT.DESCRIPTION_PLACEHOLDER')
      .subscribe((res: string) => (this.placeholder = res));
    this.translate
      .get('PROFILE.EDIT.APPLY_BUTTON')
      .subscribe((res: string) => (this.applyBtn = res));
    this.translate
      .get('PROFILE.EDIT.CANCEL_BUTTON')
      .subscribe((res: string) => (this.cancelBtn = res));
    this.translate
      .get('PROFILE.EDIT.CHANGES_SAVED')
      .subscribe((res: string) => (this.changesSaved = res));

    this.description = this.user.description;
  }

  public fileChangeHandler(event: any) {
    this.fileEditorService
      .open(event, {
        aspectRatio: 4 / 4,
        autoCropArea: 1,
        resizeToWidth: 500,
